...
sparse-matrix data structures for storage; none offers a uniform, queryable
...
verified cell-by-cell against the dense table, it stays within a small factor of
...
The size of this table is one obstacle to working with it. For an $n$-bit S-box, the DDT
...
$n\approx 15$--$16$ the dense matrix can no longer be materialized on commodity
...
the primitives that can actually be analyzed.
...
self-checking artifact reproduces every measurement.\footnote{Artifact anonymized
for review.}
...
higher-order attacks; for that reason, several cryptanalytic tasks need
...
correspondence~\cite{kimura}.

\paragraph{A worked example.}
To illustrate the construction and its reduction, consider the $3$-bit S-box of
the SEA cipher~\cite{sea}, whose DDT is shown in Table~\ref{tab:sea}. Because the S-box is
...
is omitted for clarity. We verified that it reproduces all $64$ cells of the DDT
exactly.
...
a contiguous array of states, each occupying four fields that hold the indices of
its successors. The first $\lceil\Delta/2\rceil + 1$ entries are sink states whose
four fields all point to themselves: index $0$ is the dead state, and index $i$
(for $1\le i\le\lfloor\Delta/2\rfloor$) is the accepting state for differential
value $2i$. A traversal that reaches such a sink stays there, so the index alone
identifies the cell's value and no auxiliary value table is needed. Each pointer
is bit-packed to $\lceil\log_2|S|\rceil$ bits, the smallest width that can index
the $|S|$-entry array (internal states and sinks combined). We report the
\emph{state ratio}, the dense cell count $2^{2n}$ divided by the number of
automaton states.
...
strong AES-like~\cite{rijndael} S-boxes with deliberately weak or highly structured ones; we
...
$\Delta=64$ SKINNY $8$-bit S-box~\cite{skinny} (shared by Romulus, Remus, and ForkSkinny) is
stored in $915$ states ($71.6\times$), and the deliberately weak CSS S-box
($\Delta=128$) in only $240$ states ($273\times$). The full per-S-box results
...
\paragraph{Storage compared to standard baselines.}
Table~\ref{tab:memory} translates these state counts into bytes and pits the
automaton against two baselines. \emph{Dense} stores every cell using
$\lceil\log_2(\Delta+1)\rceil$ bits, the minimum needed for the value range and
...
$\lceil\log_2 d\rceil$-bit index into a dictionary of the $d$ distinct values;
it exploits the abundance of zeros but not the deeper structure of the table.
The automaton dominates both on highly structured S-boxes, CSS in under
$1$~KiB against $64$~KiB and $8.6$~KiB, SKINNY-8 in $4.5$~KiB against $56$~KiB
and $14$~KiB, DryGASCON256 in $6.5$~KiB against $256$~KiB and $45$~KiB, and
stays within a small factor of bitmap on less structured ones; on AES the
bitmap is sharper ($12$~KiB versus $34$~KiB), reflecting that the AES DDT has
many zeros but little subtree sharing to exploit beyond them. Neither baseline
supports the query operations of Section~\ref{sec:applications}.
...
bit-packed value-range minimum, $\lceil\log_2(\Delta+1)\rceil$ bits per cell;
\emph{bitmap} is a $2^{2n}$-bit presence map plus a small value dictionary.
...
\midrule
SEA            & 3 & 2   &     18~B  &     16~B  &     12~B \\
Noekeon        & 4 & 4   &     75~B  &     96~B  &     45~B \\
PRESENT        & 4 & 4   &    117~B  &     96~B  &     45~B \\
SKINNY (4-bit) & 4 & 4   &    105~B  &     96~B  &     45~B \\
PRINCE         & 4 & 4   &    144~B  &     96~B  &     46~B \\
GIFT           & 4 & 6   &    141~B  &     96~B  &     58~B \\
Midori $S_{b0}$& 4 & 4   &    156~B  &     96~B  &     45~B \\
Ascon          & 5 & 8   &    374~B  &    512~B  &    209~B \\
Fides (5-bit)  & 5 & 2   &    340~B  &    256~B  &    190~B \\
APN (6-bit)    & 6 & 2   &  1.6~KiB  &  1.0~KiB  &    746~B \\
WAGE           & 7 & 8   &  8.1~KiB  &  8.0~KiB  &  3.6~KiB \\
AES            & 8 & 4   & 34.2~KiB  & 24.0~KiB  & 12.0~KiB \\
Camellia       & 8 & 4   & 34.2~KiB  & 24.0~KiB  & 12.0~KiB \\
CLEFIA $S_0$   & 8 & 10  & 35.7~KiB  & 32.0~KiB  & 17.3~KiB \\
Skipjack       & 8 & 12  & 35.9~KiB  & 32.0~KiB  & 17.6~KiB \\
Kuznyechik     & 8 & 8   & 35.5~KiB  & 32.0~KiB  & 14.7~KiB \\
Fantomas       & 8 & 16  & 28.9~KiB  & 40.0~KiB  & 15.7~KiB \\
SKINNY (8-bit) & 8 & 64  &  4.5~KiB  & 56.0~KiB  & 13.6~KiB \\
CSS            & 8 & 128 &  0.9~KiB  & 64.0~KiB  &  8.6~KiB \\
DryGASCON256   & 9 & 128 &  6.5~KiB  &   256~KiB & 44.6~KiB \\
\bottomrule
\end{tabular}
\end{table}
